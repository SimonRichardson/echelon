...
const (
	defaultKeysKey   string = "uOO6c1OIKZ81z8SwPE7LXtDIBjfnOMOq0I5JmiDM"
...
func (c *cluster) Select(key bs.Key, field bs.Key) <-chan t.Element {
	return c.selectCommon(key, func(conn redis.Conn, key bs.Key) ([]s.KeyFieldScoreTxnValue, error) {
		return wrapSelection(selection(conn, key, field))
	})
}

func (c *cluster) SelectRange(key bs.Key, limit int, sizeExpiry s.KeySizeExpiry) <-chan t.Element {
	return c.selectCommon(key, func(conn redis.Conn, key bs.Key) ([]s.KeyFieldScoreTxnValue, error) {
		return selectionWithRange(conn, key, limit, sizeExpiry[key])
	})
}
...
func (c *cluster) selectCommon(key bs.Key,
	f func(redis.Conn, bs.Key) ([]s.KeyFieldScoreTxnValue, error),
) <-chan t.Element {
	out := make(chan t.Element)
	go func() {
		...
		go func() {
			...
			var (
				elements []t.Element
				result   = []s.KeyFieldScoreTxnValue{s.KeyFieldScoreTxnValue{Key: key}}
			)
			if err := c.pool.With(key.String(), func(conn redis.Conn) (err error) {
				result, err = f(conn, key)
				return
			}); err != nil {
				elements = errorElementsFromKeyFieldScoreTxnValue(result, err)
			} else {
				elements = successElementsFromKeyFieldScoreTxnValue(result)
			}
...
func errorElementsFromKeyFieldScoreTxnValue(keys []s.KeyFieldScoreTxnValue, err error) []t.Element {
	elements := make([]t.Element, 0, len(keys))
	for _, k := range keys {
		elements = append(elements, t.NewErrorElement(k.Key, err))
	}
...
func successElementsFromKeyFieldScoreTxnValue(keys []s.KeyFieldScoreTxnValue) []t.Element {
	// bucketize
	buckets := map[bs.Key][]s.KeyFieldScoreTxnValue{}
	for _, k := range keys {
		buckets[k.Key] = append(buckets[k.Key], k)
	}

	elements := []t.Element{}
	for k, v := range buckets {
		elements = append(elements, t.NewKeyFieldScoreTxnValue(k, v))
	}
	return elements
}
...
func wrapSelection(sel s.KeyFieldScoreTxnValue, err error) ([]s.KeyFieldScoreTxnValue, error) {
	if err != nil {
		return nil, err
	}
	return []s.KeyFieldScoreTxnValue{sel}, nil
}
