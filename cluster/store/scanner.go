...
func keys(conn redis.Conn, batchSize int) ([]bs.Key, error) {
	...
	for {
		...
		if n := len(values); n != 2 {
			return result, typex.Errorf(errors.Source, errors.UnexpectedResults, "Received %d values from redis, expected 2", n)
		}
...
func members(conn redis.Conn, key bs.Key) ([]bs.Key, error) {
	m, err := redis.Strings(conn.Do("HKEYS", prefix+key+insertSuffix))
	if err != nil {
		return nil, err
	}
...
func cardinality(conn redis.Conn, key bs.Key) ([]s.KeyCount, error) {
	res, err := redis.Int(conn.Do("HLEN", prefix+key+insertSuffix))
	if err != nil {
		res = 0
	}
	return []s.KeyCount{
